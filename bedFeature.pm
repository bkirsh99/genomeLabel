...
#print Dumper LABELS->{CIS_REG_MODULE}[0];
	if ($file eq LABELS->{EXON}[0] || $file eq LABELS->{INTRON}[0] || $file eq LABELS->{INTERGENIC}[0] || $file eq LABELS->{TRANSCRIPT}[0] || $file eq LABELS->{CODING}[0] || $file eq LABELS->{NONCODING}[0]){
		my $id = defined $self->{original}->{ATTRB}->{transcript_id} ? join(':', $self->{original}->{ATTRB}->{transcript_id}, $self->{original}->{ATTRB}->{gene_name}) : join(':', $self->{original}->{ID}, $self->{original}->{ATTRB}->{gene_name});
		$self->{name} = $self->{original} eq 'BEDTools Complement' ? 'intergenic' : $id;
