...
sub manipulate_Bed{
	...
	system "awk 'BEGIN{OFS=\"\\t\";} {gsub(/intron/,\"transcript\",\$4)}; {print}' $var->{TRANSCRIPT}[0] | gzip > $var->{TRANSCRIPT}[0].gz";
		...
		foreach my $fref (keys %file){
                	system "cat $_ | $command > tmp && mv tmp $_"; #sanity check
        	}
	}

	system "cat $var->{CODING}[0] $var->{NONCODING}[0] > $var->{EXON}[0] | rm $var->{CODING}[0] $var->{NONCODING}[0]";# if ((-f $var->{EXON}[0]) && (-f $var->{CODING}[0]) && (-f $var->{NONCODING}[0])); 
}
