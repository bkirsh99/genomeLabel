...
print "Creating raw annotation files and GIGGLE directory\n";
...
if (@regulator | @repeat | @felement){
	print "Creating filtered annotation files\n";
...
sub updateHistory{
        ...
        if (-e $history && `cat $history | wc -c` != 0){ #exists and is not empty
		...
		} else { #Segway and the rest require different dowload commands
	...
	appendInput(); #extend region/biotype in history.txt
}
