...
print "\nWould you like to get summary statistics for the labels in $region? (Y/N)\n";

my $summaryAns = <STDIN>;
if ($summaryAns =~ m/^[Y]$/i){
	system "./getSats.pl $region";
	if(-e $statsFile){ 
		print "\nSummary statistics successfully extracted and saved as 'summary.txt' file to regional directory.\nWould you like to print the results (Y/N)?";
		my $printAns = <STDIN>;
		...
		print "Would you like to create track files for data visualization (Y/N)?";
		my $trackAns = <STDIN>;

		if ($trackAns =~ m/^[Y]$/i){
			system "./makeTracks.pl $region";
			if (-d $trackFileDir){
				print "\nTracks successfully created and saved as '.bed' files to 'trackFiles' in regional directory.\nWould you like to visualize your results (Y/N)?";
				my $viewerAns = <STDIN>;
			}
		}
	} 
