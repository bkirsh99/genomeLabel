...
sub load{
		...
		unless ($self->{fixed}){
			...
			"gunzip -c $file | awk -F \"\\t\" 'BEGIN{OFS=\"\\t\";} ((\$9~/NM/ || \$9~/NR/) && \$1==\"$self->{chrom}\" && ((\$5 >= $self->{chromStart} && \$5 <= $self->{chromEnd}) || (\$4 >= $self->{chromStart} && \$4 <= $self->{chromEnd})))' > temp" ,
			"./gffextract.pl -i temp -k longest | gzip > $tmp && rm temp");
		}
	} else {
		if (defined $self->{hub}){
