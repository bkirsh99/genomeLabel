...
#print Dumper ref($retrieve);
#appendInput($retrieve);
#print Dumper ref(retrieve($history));
updateHistory($history);
appendInput($history);

#> STEP 2: Create raw files and the giggle_files direcory given the input region and chosen biotype: 
print "\nCreating raw annotation files and GIGGLE directory\n";
system "./make.pl $history $path $biotype $out $biotypeR";

#> STEP 3: Create filtered files given the chosen repeats, regulators, and genome segments:
if (@regulator | @repeat | @felement){
	print "\nCreating filtered annotation files\n";
	system "./make.pl $history $path $biotype $out $biotypeR $repfilter $felefilter $crmodfilter";
}
...
sub updateHistory{
	my $f = shift;
   	...
   	if (-e $f && `cat $f | wc -c` != 0){ #exists and is not empty
		my $retrieve = retrieve($f); #stores previous inputs (ARRAY OF TWO HASHES) 
		...
		if ( scalar(@{ $$init{'all'} }) == scalar(@{ $$init{'biotype'} }) ){
			...
			for($loop_index = 0; $loop_index <= $#{ $$init{'all'} }; $loop_index++) { #compare regions
				...
				#print "$s1 and $e1 and $biotypeS\n";
				#print "$s2 and $e2 and $biotypeS\n" if (defined $s2 && defined $e2); 
				system "./init.pl $history $chr $s1 $e1 $biotypeS";
				system "./init.pl $history $chr $s2 $e2 $biotypeS" if (defined $s2 && defined $e2);
			}
		} else { #New biotype, Segway and the rest require different dowload commands
...
sub appendInput{
	...
	if( ref($f) eq 'HASH'){ #first time, must add hash of previous inputs to array
		push @$AoA, $coords;
...
sub adjustCoord{
		...
		#@new = [ $chr, $coordinates[0], $coordinates[1] ];
	}
        return @new;
}
